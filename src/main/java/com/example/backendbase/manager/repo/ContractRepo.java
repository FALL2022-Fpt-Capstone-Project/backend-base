...
public interface ContractRepo extends JpaRepository<Contracts, Long> {
    ...
    @Query("SELECT c FROM Contracts c WHERE c.endDate < :now OR c.isDisable = TRUE AND c.contractTerm = 1 AND c.groupId = :groupId")
    List<Contracts> findAllExpiredContract(@Param("now") Timestamp now, @Param("groupId") Long id);
}
